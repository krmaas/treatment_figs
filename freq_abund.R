...
f$group <- 1
f.s.d <- rbind(f,f.s)
f.s.d$group <- as.factor(f.s.d$group)
str(f.s.d)

fs.plot <- ggplot(f.s.d, aes(x=f.pres, y=f.mean, alpha=group, color=group))+
  geom_point(size=4)+
...
b$group <- 1
b.s.d <- rbind(b,b.s)
b.s.d$group <- as.factor(b.s.d$group)
#check that the groups copied over ok
min(b.s.d$group)
max(b.s.d$group)

bs.plot <- ggplot(b.s.d, aes(x=b.pres, y=b.mean, alpha=group, color=group))+
  geom_point(size=4)+
  scale_color_manual(values=c("black", "red"))+
  scale_y_log10(name="Mean OTU abundance")+
  scale_x_continuous(name="Sample occurance")+
